...
// Ambil data KHS berdasarkan role
try {
    if ($role == 'mahasiswa') {
        $sql = "
    ...
    // Buat HTML untuk PDF
    $html = '<h1 style="text-align: center; font-size: 24px; margin-bottom: 20px;">Kartu Hasil Studi (KHS)</h1>';
    ...
    $dompdf->render();
    $dompdf->stream("khs.pdf", array("Attachment" => false));
